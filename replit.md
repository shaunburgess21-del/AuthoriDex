...
- **Scoring Engine**:
    ...
    - **GDELT Hardening (Feb 2026)**: Circuit breaker (3 failures → 5-min pause), stale cache fallback, candidate gating (top 25 by rank + top 25 by wiki = ~35-50 fresh API calls instead of ~100), 2-min time budget per batch. **Adaptive spacing**: Inter-request delay starts at 5.5s, increases by 1.5x on failure, decreases by 0.85x after 3 consecutive successes (floor 3s, ceiling 30s), reset at batch start. **Proactive cache reuse**: Skips GDELT re-fetch when cached data is <90 min old (extended to 3h during degraded/outage/recovery state), reducing total API volume per run.
