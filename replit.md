...
- **Scoring Engine**:
    ...
    - **Mock Data Safeguard** (Feb 2026): The storage layer rejects writes if average fameIndex < 50,000 (real data is 100k-600k range). API endpoints no longer write to trending_people - only the ingestion job writes real scores. Prevents accidental mock data corruption.
