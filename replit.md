...
- **Scoring Engine**:
    ...
    - **Score Stabilization**: A multi-layered approach prevents wild score fluctuations using dynamic rate limiting based on source corroboration (max 8% steady-state, 12% for 1 source spiking, 20% for 2 sources, 35% for 3 sources), robust spike detection, and dynamic EMA alpha (0.10 / 0.15 / 0.22).
    - **Auto Catch-Up Mode** (Feb 2026): Gap-driven dynamic rate boosting replaces the old time-based recalibration mode. When median(abs(raw-final)/final) across all celebrities exceeds 8%, catch-up mode activates with 2.5x cap multiplier and 1.8x alpha multiplier. Auto-disengages when median gap drops below 4% for 2 consecutive runs. State is **DB-persisted** (in `api_cache` with key `system:catchup_state`) so restarts don't reset the exit streak counter. This prevents the leaderboard from freezing after outages or algorithm changes while maintaining stability during normal operation.
    - **Convergence Metrics** (Feb 2026): Health summary now includes `medianGapPct`, `p90GapPct`, `catchUpExitStreak`, `catchUpEnteredAt`, `capsUsed` (effective caps with multipliers), and `alphaUsed` (effective alphas with multipliers). Healthy targets: medianGapPct < 5%, rateLimitedPct < 25%.
