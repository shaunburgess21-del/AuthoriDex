...
- **Payout Pipeline Hardening**: `settleMarketBets()` and `voidMarketBets()` write immutable `credit_ledger` entries (txnTypes: `prediction_payout`, `prediction_refund`, `prediction_stake`) with idempotency keys (`payout_{marketId}_{betId}`, `refund_{marketId}_{betId}`, `stake_{marketId}_{betId}`). Market-level idempotency guard: returns early if market status is already RESOLVED/VOID. Bet placement (`POST /api/open-markets/:slug/bet`) writes `prediction_stake` ledger entries in a transaction, with server-side rate limiting (max 10 bets/min per user) and max stake cap (500 credits). Pool conservation tracked via `remainder` field. `credit_ledger.userId` FK to `users` removed (supports both legacy users and Supabase profiles). E2E test endpoint gated by `ENABLE_TEST_ENDPOINTS=true` env var. Test script: `npx tsx scripts/test-payout-e2e.ts`. Ledger baseline smoke test: `npx tsx scripts/test-ledger-baseline.ts` (5 checks: profileCredits, ledgerSum, entryCount, drift, idempotency).
- **User Predictions Endpoint**: `GET /api/me/predictions` returns user's bet history with result status (won/lost/refunded/pending), payout amounts, and entry labels. Used by PredictPage to display bet outcomes on resolved market cards.
- **Ledger Baseline**: Profile creation (`/api/profile/sync`) writes an `initial_grant` credit_ledger entry (`txnType: 'initial_grant'`, amount: 1000, `idempotencyKey: 'initial_grant_${userId}'`) alongside the profile insert. All existing profiles were backfilled. This ensures reconciliation is meaningful from day one.
- **Alert Tripwires**: `[CREDIT DRIFT]` logged per-user when `GET /api/admin/credit-reconciliation` detects `profiles.predictCredits != SUM(credit_ledger.amount)`. `[PAYOUT REMAINDER LARGE]` logged when `settleMarketBets()` produces `abs(remainder) > 1` (early-warning for payout math bugs).
...
- **Admin Ops Summary**: `GET /api/admin/ops-summary` returns pendingCount, closingSoonCount (markets ending within 2h), resolverLastRunAt, resolverHealthy, driftUserCount. Displayed as 4 always-visible summary cards at top of admin dashboard.
- **Admin Settlement Center** (`client/src/components/admin/AdminSettlementCenter.tsx`): Pending queue with real data, resolution dialog with payout preview per outcome, void flow with reason, settlement history. Sanity warnings: `no_bets`, `stuck` (>30min pending), `concentration` (>60% single user). "Already settled" responses show informational toast instead of error.
- **Admin User Credit History** (`client/src/components/admin/AdminUserCreditHistory.tsx`): Paginated dialog (50 entries/page) showing profile stats, credit drift alert, color-coded ledger history with TxnType badges. Backend: `GET /api/admin/users/:id/credit-history?page=N`.
