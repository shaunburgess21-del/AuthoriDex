...
import type { BrandLogoVariant } from "@shared/brand-tokens";
import { BRAND_LOGO_GRADIENTS } from "@shared/brand-tokens";

const BRAND_ASSET_PNG_SIZES = [256, 512, 1024, 2048] as const;

export function AdminBrandAssetsCard() {
  const [brandAssetVariant, setBrandAssetVariant] = useState<BrandLogoVariant>("default");

  const buildBrandLogoSvgMarkup = useCallback((variant: BrandLogoVariant) => {
    const colors = BRAND_LOGO_GRADIENTS[variant];
    const defs = `<defs>
  ...
  const getBrandAssetFilenameBase = useCallback((variant: BrandLogoVariant) => {
      ...
      <CardContent className="space-y-4">
        <div className="space-y-2">
          ...
          <Select
            ...
            onValueChange={(value) => setBrandAssetVariant(value as BrandLogoVariant)}
          >
