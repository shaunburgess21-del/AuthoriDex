...
- Embedded value-vote widgets (`UnderratedOverratedCard`, `OverratedUnderratedWidget`) bypass the Phase 4 gate. Confirmed during Stage 8 manual smoke (May 2026): clicking Underrated/Overrated on a person profile or the `/vote` page Underrated/Overrated section triggers the widget's own pre-Phase-4 "Sign in to vote" toast instead of the gate's `checkVoteGate` → `navigateToLogin` path. Same bypass likely affects `ValueVoteModal` and `InductionLeaderboardSlice`. Fix: wire `useAnonBudget` + `checkVoteGate` into each widget's mutation handler, mirroring the Stage 7 pattern from `VoteDeckView`.
- Stage 8 smoke initially failed because migration `0049_anon_vote_budget` was applied to git but not to the dev DB — anon votes returned `budget: null` and the budget table stayed empty. The fail-open path in `consumeBudgetUnit` correctly prevented user-facing breakage but masked the missing migration. Stage 8 added an explicit "Apply pending migrations to dev DB" step to `phase4-manual-test.md` Setup section. Future improvement: consider a startup health check that warns if migration journal entries exist that aren't reflected in `schema_migrations`.
