// Dry-run scoring with Fix A (search weight = 0) + Fix B (upper-tail spread)
// applied. Reads the Phase 1 audit snapshot in audit-trend-engine-output.json
// (top 50 with raw signals captured at 2026-04-26T20:00Z) and recomputes
// fameIndex using the *current* normalize.ts (which already has Fix A + B
// committed on this branch). Diff against the persisted live fameIndex
// reveals leaderboard rank changes attributable to PR1 alone.
//
...
console.log(`Dry-run: Fix A + B applied to snapshot ${audit.generatedAt}`);
...
for (const r of rows.slice(0, 30)) {
  ...
  console.log(
    pad(r.oldRank, 6, "R") +
      ...
      pad(r.wikiVel.toFixed(1), 7, "R") +
      pad(r.newsVel.toFixed(1), 7, "R") +
      pad(r.searchVel.toFixed(1), 7, "R"),
  );
}
