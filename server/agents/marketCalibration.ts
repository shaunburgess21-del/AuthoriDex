...
export interface GainerCalibrationResult {
  totalResolved: number;
  avgWinnerFinalPrice: number | null;
  /** Winners with final price <= 1/N baseline proxy (0.15). */
  winnersPricedAtOrBelow15: number;
  winnersTotal: number;
}

export async function fetchGainerCalibration(
  lookbackDays = 30,
): Promise<GainerCalibrationResult> {
  const rows = await db.execute(sql`
    WITH base AS (
      SELECT
        pm.id AS market_id,
    ...
    SELECT
      count(*) FILTER (WHERE resolution_status = 'winner')::int AS n_winners,
      round(avg(price) FILTER (WHERE resolution_status = 'winner'), 3)::float AS avg_winner_price,
      count(*) FILTER (WHERE resolution_status = 'winner' AND price <= 0.15)::int AS winners_le_15
    FROM priced
  `);
  ...
  const row = rows.rows[0] as {
    n_winners: number;
    avg_winner_price: number | null;
    winners_le_15: number;
  } | undefined;

  return {
    totalResolved: Number(row?.n_winners ?? 0),
    avgWinnerFinalPrice: row?.avg_winner_price ?? null,
    winnersPricedAtOrBelow15: Number(row?.winners_le_15 ?? 0),
    winnersTotal: Number(row?.n_winners ?? 0),
  };
}
